const API="https://nslfyglqamrhbqfdwftr.supabase.co/functions/v1/mova-console?key=mova_console_demo_v1&mode=fleet";
...
async function boot(){try{const r=await fetch(API);const d=await r.json();if(!r.ok)throw new Error(d.error||"Unable to load dashboard");render(d.fleet)}catch(e){root.innerHTML=`<div class="notice">${e.message}</div>`}}
function render(f){
 const m=f.metrics;
 const max=Math.max(...f.weekly_sessions.map(x=>x.value));
 root.className="";
 root.innerHTML=`
 ...
 <div class="metrics">
  ${metric("Drivers",m.drivers,"Fleet population")}
  ${metric("Activated",m.activated,"Used MOVA at least once")}
  ${metric("Activation",m.activation_rate+"%","Fleet adoption")}
 ...
 ${f.sites.map(s=>`<tr><td><strong>${s.name}</strong></td><td>${s.activation}%</td><td>${s.sessions}</td><td><span class="status live">Active</span></td></tr>`).join("")}
